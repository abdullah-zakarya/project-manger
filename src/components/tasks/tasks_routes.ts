...
const validTaskInput = [
  ...
  body('projectId').trim().notEmpty().withMessage('It should be required'),
  body('userId').trim().notEmpty().withMessage('It should be required'),
  ...
  body('estimatedEndTime')
    ...
    .notEmpty()
    .withMessage('It should be required'),
  body('estimatedEndTime').custom((value) => {
  ...
  body('estimatedEndTime').custom((value, { req }) => {
    if (!checkValidDate(value)) {
      throw new Error('Invalid date format YYYY-MM-DD HH:mm:ss');
    }
...
export class TaskRoutes {
  private baseEndPoint = '/api/v1/tasks';
